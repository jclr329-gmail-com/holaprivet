...
@section('titulo', 'Создать аккаунт')

@section('cuerpo')
<div class="ancho estrecho">
  <h1 class="titulo-cuenta">Создать аккаунт</h1>
  <p class="sub-cuenta">Бесплатно, как и весь курс. Аккаунт хранит ваш прогресс
  ...
  <form method="POST" action="{{ route('registro') }}" class="forma">
    @csrf
    <label>Имя
    ...
    <label>Пароль <span class="pista">(не короче 8 знаков)</span>
    ...
    <button class="boton" type="submit">Создать аккаунт</button>
  </form>

  <p class="pie-forma">Уже есть аккаунт? <a href="{{ route('login') }}">Войти</a></p>
</div>
@endsection
